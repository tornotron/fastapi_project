...
class TradeApp(EWrapper, EClient):
    ...
    def position(self, account, contract, position, avgCost):
        ...
        self.pos_df = pd.concat(
            [self.pos_df, pd.DataFrame.from_dict(dictionary)],
            ignore_index=True,
        )
    ...
    for ticker in tickers:
        ...
        if len(pos_df.columns) == 0:
            if (
                ...
                app.placeOrder(
                    order_id, usTechStk(ticker), marketOrder("BUY", quantity)
                )
                time.sleep(5)
                try:
                    pos_df = app.pos_df
                    ...
                    app.placeOrder(
                        ...
                        stopOrder(
                            "SELL",
                            sl_q,
                            round(df["Close"].iloc[-1] - df["atr"].iloc[-1], 1),
                        ),
                    )
